...
class DocReader():
    ...
    def input_data(self,objDir,subjDir,percentTest,cutoff):
        ''' 
        Input:
          objDir: String. dir of objective text files
          subjDir: String. dir of subjective text file
        ...
        '''
        pathLabelPairs={}
        for objPath in glob.glob(objDir+'*'):
            pathLabelPairs.update({objPath:(0,1)})
        for subjPath in glob.glob(subjDir+'*'):
            pathLabelPairs.update({subjPath:(1,0)})
...
if __name__ == '__main__':
    ...
    '''
    objDir : folder directory of the objective text files
    subjDir : folder directory of the subjective text files
    ...
    trainX,trainY,testX,testY = reader.input_data(objDir=objDir,
                                                  subjDir=subjDir,
                                                  percentTest=.1,
                                                  cutoff=15)
